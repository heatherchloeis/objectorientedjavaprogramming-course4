...
public class EarthquakeCityMap extends PApplet {
	...
	private void hideMarkers(String type) {
		...
		if (type.equals("quake")) {
			for (Marker marker : quakeMarkers) {
				if (marker != lastClicked) {
					marker.setHidden(true);
				}
			}
			threatDist = ((EarthquakeMarker)lastClicked).threatCircle();
			for (Marker marker : cityMarkers) {
				cityDist = lastClicked.getDistanceTo(marker.getLocation());
				if (cityDist <= threatDist) {
		...
		if (type.equals("city")) {
			...
			for (Marker marker : quakeMarkers) {
				threatDist = ((EarthquakeMarker)marker).threatCircle();
				cityDist = marker.getDistanceTo(lastClicked.getLocation());
				if (cityDist <= threatDist) {
					marker.setHidden(false);
				} else {
					marker.setHidden(true);
