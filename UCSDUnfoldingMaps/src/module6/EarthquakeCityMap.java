		...
		if (lastClicked != null) {
			...
			for (Marker m : quakeMarkers) {
				...
				if (!quakeMarker.isHidden()) {
					...
					String time = quakeMarker.getStringProperty("age");
					if ("Past Hour".equals(time) || "Past Day".equals(time)) {
						countRecent++;
					}
				}
				float magAvg = magSum / countNearby;
				...
				rect(xbase, ybase, 150, 110);
				
				fill(0);
				textAlign(LEFT, CENTER);
				textSize(12);
				text("Earthquake Info", xbase+25, ybase+25); 
				text("Nearby: " + countNearby, xbase+25, ybase+45);
				text("Avg Mag: " + df.format(magAvg), xbase+25, ybase+65);
				text("Recent: " + countRecent, xbase+25, ybase+85);
			}
		}
	}
